...
export class AuthService {
    ...
    public signup(username: string, password: string, email: string, 
        name: string, lastName: string, birthDate: string, phone: string): Observable<boolean> { 
        
        return this.http.post<{type: boolean, data: string}>(this.URL_API + '/api/signup', {username, password, email, name, lastName, birthDate, phone})
            .pipe(map(resultado => {                
    ...
    login(username: string, password: string): Observable<{ type: boolean, data:string, token: string }> {
        return this.http.post<{type: boolean, data:string, token: string}>(this.URL_API + '/api/login', {username, password})
            .pipe(map (resultado => {
                if (resultado.type)
    ...
    comprobarToken(): Observable<boolean> {
        return this.http.get<{tokenValido: boolean}>(this.URL_API + '/api/comprobar-token?token=' + this.token)
        .pipe(
            map(resultado => {
